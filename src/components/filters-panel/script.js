if (document.documentElement.clientWidth >= 768) {
  const filtersPanelLists = document.querySelectorAll(
    "#filters-panel__list-js"
  );
  filtersPanelLists.forEach(function (list) {
    const filtersPanelItems = list.querySelectorAll(".filter-item");
    const filtersPanelShow = list.querySelector(".filters-panel__more");

    if (filtersPanelItems.length > 6) {
      filtersPanelShow.style.display = "flex";
      let listVisible = false;
      for (let i = 6; i < filtersPanelItems.length; i++) {
      ...
      filtersPanelShow.addEventListener("click", function () {
        if (listVisible) {
          ...
          for (let i = 6; i < filtersPanelItems.length; i++) {
            filtersPanelItems[i].style.display = "block";
          }
          filtersPanelShow.innerHTML = `<span>Спрятать фильтры</span> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
