...
$arParams['SORT_PROP'] = $_SESSION['REVIEW_SORT_PROP'] ? $_SESSION['REVIEW_SORT_PROP'] : 'DateFormated';
	...
	foreach ($arResult["PagesComment"] as $page => &$comments) {
		foreach ($comments as &$comment) {
			...
			if(isset($arComments[$comment['ID']])) {
				...
				if($comment['UF_ASPRO_COM_RATING'] && !in_array($comment['AUTHOR_ID'], $authorIds)) {
					$arCommentsRating['COUNT']++;
					$arCommentsRating['VALUE'] += $arComments[$comment['ID']]['UF_ASPRO_COM_RATING'];
					$authorIds[] = $comment['AUTHOR_ID'];
				}
			}
