		...
		function BX_SetPlacemarks_<? echo $arParams['MAP_ID'] ?>(map) {
			if (typeof window["BX_YMapAddPlacemark"] != 'function') {
				...
				(function (d, s, id) {
					...
					// js.src = "/local/templates/html/components/bitrix/map.yandex.view/map/script.js";
			...
			map.geoObjects.events.add('click', function (e) {
				setTimeout(function () {
					const ymaps = document.querySelector('.ymaps-b-balloon .ymaps-b-balloon__close');
					ymaps.classList.add('close_custom');
					ymaps.innerHTML = closeSVG;
				}, 100);
			});
			...
			document.querySelector('.block_container .items .item')?.classList.add('initied');
		}

		document.addEventListener("DOMContentLoaded", function () {
			document.querySelector('.contacts-stores .item .top-wrap .show_on_map span[data-coordinates]')?.addEventListener('click', function (e) {
				var arCoordinates = e.target.dataset.coordinates.split(','),
					mapOffsetTop = document.querySelector('.contacts-page-map').offsetTop;

				window.scrollTo({
					top: mapOffsetTop - (isMobile ? 120 : 260),
					left: 0,
					behavior: "smooth",
				});
			...
			document.querySelectorAll('.contacts-region__list .contacts-region__card-map')?.forEach(x=>{
				x.addEventListener('click', function (e) {

					const arCoordinates = e.target.dataset.coordinates.split(',')
					const mapOffsetTop = document.querySelector('.contacts-map').offsetTop;
